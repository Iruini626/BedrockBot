...
```bash
BedrockBot$ sam build --use-container
```
...
```bash
BedrockBot$ sam logs -n StockCheckerFunction --stack-name "bedrockbot" --tail
```
...
```bash
BedrockBot$ pip install -r tests/requirements.txt --user
# unit test
BedrockBot$ python -m pytest tests/unit -v
# integration test, requiring deploying the stack first.
# Create the env variable AWS_SAM_STACK_NAME with the name of the stack we are testing
BedrockBot$ AWS_SAM_STACK_NAME="bedrockbot" python -m pytest tests/integration -v
```
...
```bash
sam delete --stack-name "bedrockbot"
```
